...
def train_one_epoch(__C,
    ...
    for ith_batch, data in enumerate(loader):
        ...
        #ref_iter, image_iter, box_iter, gt_box_iter, info_iter,_ = data
        ref_iter, image_iter, box_iter, gt_box_iter, info_iter= data#cui1 dataloader返回中没有最后这个_
        ...
        with torch.no_grad():
            ...
            if epoch == 0:  # 第 0 个 epoch：关掉
                model.w_comp = torch.tensor(0.0, device=ref_iter.device)
            if epoch == 2 and ith_batch == 0:  # 第 2 个 epoch 再打开
                model.w_comp = torch.tensor(0.0003, device=ref_iter.device)



        if scalar is not None:
            with th.cuda.amp.autocast():
                loss = net(image_iter, ref_iter)##cui1 返回相似度


        else:
            loss = net(image_iter, ref_iter)##cui1
...
def main():
    ...
    #parser.add_argument('--config', type=str, default='/root/autodl-tmp/DViN/config/refcoco+.yaml')
    #parser.add_argument('--config', type=str, default='/root/autodl-tmp/DViN/config/refcocog.yaml')
    print("q")
